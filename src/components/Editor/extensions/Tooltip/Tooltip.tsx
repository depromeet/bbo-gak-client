...
function ShortcutKey({ children }: { children: string }): JSX.Element {
  const className =
    'inline-flex items-center justify-center w-20 h-20 p-4 text-[0.625rem] rounded font-semibold leading-none border border-neutral-200 text-neutral-500 border-b-[1.5px]';
...
export function Tooltip({ children, enabled = true, title, shortcut, tippyOptions = {} }: TooltipProps) {
  const renderTooltip = useCallback(
    (attrs: TippyProps) => (
      <span
        className="flex items-center gap-8 px-10 py-4 bg-white border border-neutral-100 rounded-lg shadow-sm z-[999]"
        tabIndex={-1}
